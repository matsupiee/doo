...
| `mission_participant`            | そのミッションを「やる」人。参加はこの行の作成     |
...
参加に承認・招待の概念は持たせない。他人のやりたいことを見て自分から乗るだけなので、
...
非公開フラグは持たない。他人のやりたいことを見て参加するのがアプリの中心なので、
...
参加追加は制約を緩めるのではなく、制約を満たす行を先に作る操作になる。記録後の状態は、
本人が自分で参加してから達成した場合とまったく同じになる。

自動で参加者にされた人には通知を出す（下記）。知らないうちに参加者一覧に載るため。
