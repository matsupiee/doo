...
- **ミッションから抜けると、その人の達成記録も消える。** `mission_participant` への FK が
  cascade なので、参加をやめると `mission_completion_participant` の行が道連れになる
  （一時 DB で確認済み）。ミッション自体の削除では正しい挙動だが、「抜ける」導線を作るなら、
  達成がある人の退出を API で禁止するか、`left_at` による論理削除にするかを決める必要がある。
  なお「参加していない人を共同達成に含める」（当日その場で誘われた人など）は、
  達成の記録時にその人を先に join させる形になる。
